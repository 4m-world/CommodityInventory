...
using MyInventoryApp.Droid.Extensions;
...
namespace MyInventoryApp.Droid.Renderers
{
    ...
    {
        const int DelayBeforeTabAdded = 10;
        ...
        public BottomTabbedPageRenderer(Context context)
            : base(context)
        {
        }
        ...
        void SetTab(TabLayout.Tab tab, string name)
        {
            try
            {
        ...
        void OnTabRemoved(object sender, ElementEventArgs e)
        {
            e.Element.PropertyChanged -= OnTabbedPagePropertyChanged;
            BadgeViews.Remove(e.Element);
        }

        async void OnTabAdded(object sender, ElementEventArgs e)
        {
